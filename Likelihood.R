  ...
  for(ii in c(1:length(iss))){
    for(nn in c(1:length(nndatasets))){
      for(kk in c(1:k)){
        ...
        o_s_i_cpdag<-as(o_s_i_cpdag_adj,"graphNEL")
        
        # if(shd(true_i_cpdag,o_s_i_cpdag)>0){
        #   plot(G)
        #   break
        # }
        
        
        SHD_true_skeleton[nn,ii,ss,kk]<-2*shd(true_i_cpdag,o_s_i_cpdag)/(sum(true_i_cpdag_adj)+sum(o_s_i_cpdag_adj))
        print(kk)
      }
    }
  }
...
par(mfrow=(c(1,2)))
for(nn in c(1:4)){
  print(mean(SHD_true_skeleton[nn,,,]))
}
